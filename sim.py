...
class Rexarm:
    ...
    def __init__(self, joints, gripper):
        ...
        self.angle_limits = np.array([
            ...
            [-116.0, 107.0],
            [-180, 179.99],
            [-135.0, 135.0], ], dtype=np.float) * D2R
...
class Simulator:
    ...
    def update_robot(self, rexarm):
        self.joints = np.copy(rexarm.get_positions)
    ...
    def run(self):
        ...
        Tlist = []
        for i in range(5):
            Tlist.append(FK_dh(self.joints, i))
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
        vis.draw_axes(np.array([0, 0, 0]), np.array(
